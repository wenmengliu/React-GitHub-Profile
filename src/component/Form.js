...
const Form = (props) => {
  ...
  const handleSubmit = (event) => {
    ...
    axios.get(`https://api.github.com/users/${username}`).then((resp)=>{
      ...
      alert(`Could not retrieve data for username "${username}"`)
    })
  }
    ...
    <form onSubmit = {handleSubmit} className = "w-full max-w-sm mx-auto">
      <div className = "mt-10 flex">
        <input
          ...
          className = "form-input block w-full py-2 px-3 border border-gray-300 shadow-sm focus:outline-none focus:shadow-outline-blue focus:border-blue-300 transition duration-150"
          required
        />
        <button
          type = "submit"
